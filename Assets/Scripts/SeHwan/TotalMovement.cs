...
public class TotalMovement : MonoBehaviour
{
    ...
    void SetInGamePlayerList() //순서대로, 포수-투수-1루수-우익수-2루수-중견수-유격수-좌익수-3루수 순서(방향에 따른 정렬)
    {
        ...
        for (int i = 0; i < inGamObjs.Count; i++)
        {
            ...
            Rt = inGams[i].GetComponent<RectTransform>();
    ...
    void MoveTo_BALL(Vector2 start, Vector2 goal, float startTime)
    {
        ...
        if (RealTime >= startTime && (RealTime - startTime) <= time)
        {
            ...
            Debug.Log(ballObj.GetComponent<RectTransform>().anchoredPosition);
        }
    }
    ...
    void Update()
    {
        ...
        if (newBALL.HomeRun)
        {
            Rt = ballObj.GetComponent<RectTransform>();
        ...
        {
            if(newBALL.TimeList[level] < BallRealtime)
            {
        ...
        MoveTo_BALL(newBALL.CaseLocation[0], BaseLocation[1], newBALL.CaseTime[0] + 1); //여기서는 수비수가 공을 잡은 후 1초 뒤에 2루(BaseLocation[1])로 던집니다.
